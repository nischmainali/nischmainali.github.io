...
- contents become a slim, article-local sticky rail in the left margin on truly
  wide screens. At smaller widths the same contents return to the reading flow,
  and collapse by default on phones;
...
- ordinary Markdown footnotes remain endnotes, while the explicit `sidenote`
  shortcode supplies optional Tufte-style right-margin notes on wide screens and
  tap-to-reveal inline notes at smaller widths;
