...
It does not copy the reference's global sidebar, sticky contents rail, enormous
bold titles, stark palette, or mobile composition. The adaptation should feel
like a scholarly field note typeset directly onto the existing lokta sheet.

Its governing choices are:

- the title, subtitle, and floral mark remain in the wide editorial field while
  prose retains the established 720 px measure;
- published and meaningfully different revised dates, reading time, categories,
  and topics form one quiet Jost metadata register, with no badges or pills;
- contents remain inline in the reading flow, use no filled panel, and collapse
  by default at narrow widths instead of becoming a sticky rail;
...
- fenced code is highlighted at build time with Chroma and receives a very small
  local copy control; article pages no longer depend on Prism or the theme's DOM
  mutation helpers;
- ordinary Markdown footnotes remain endnotes in this pass. Margin sidenotes are
  intentionally deferred because they would establish another lateral layout;
...
- `subtitle`, `lastmod`, `show_toc`, `show_reading_time`, and `math` are optional
...
desktop and approximately 390 px, including keyboard focus, code/table overflow,
copy feedback, mathematical rendering, and the absence of article console errors.
